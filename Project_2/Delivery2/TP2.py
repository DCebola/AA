...
def get_original_feats_data(_images, kernel=False):
    if kernel:
        if path.exists("data/original/original_feats_kernel.p"):
            ...
            _kernel = gaussian_kernel(50, 4.5) * 255
            for i in range(_images.shape[0]):
                _images[i] = _images[i] * _kernel
            i = 0
            for img in _images[:]:
                imsave("filtered_images/" + str(i) + ".png", img.reshape((50, 50)))
                i += 1
            imsave("filtered_images/kernel.png", _kernel.reshape((50, 50)))
            _original_feats = extract_features(_images)
            pickle.dump(_original_feats, open("data/original/original_feats_kernel.p", "wb"))
...
def get_reduced_feats_data(_images, N=18, kernel=False):
    if kernel:
        if path.exists("data/original/reduced_feats_kernel.p"):
            print("Loading features...")
            _reduced_feats = pickle.load(open("data/reduced/" + str(N) + "/reduced_feats_kernel.p", "rb"))
            print("Loaded reduced features...")
            _restored_feats = pickle.load(open("data/restored/" + str(N) + "/reduced_feats_kernel.p", "rb"))
            ...
            pickle.dump(_reduced_feats, open("data/reduced/" + str(N) + "/reduced_feats_kernel.p", "wb"))
            print("Extracted reduced features...")
            _restored_feats = extract_features(_restored_images)
            pickle.dump(_restored_feats, open("data/restored/" + str(N) + "/restored_feats_kernel.p", "wb"))
...
def experiment(_name, _feats, _labels, feature_selection=False, corr_filter=False, cluster_iter=0):
    ...
    utils.report_clusters_hierarchical(_labels[:, 0], _bisecting_report_clusters, _name + "_bisecting_kmeans_clusters.html")
...
# the ones generated when just using original images with kernel.
for n in [10, 50, 250]:
    # reduced_feats, restored_feats = get_reduced_feats_data(images, n, kernel=False)
    # experiment("experiment_reduced", reduced_feats, labels, feature_selection=False, corr_filter=True, cluster_iter=10)
    # experiment("experiment_restored", restored_feats, labels, feature_selection=False, corr_filter=True, cluster_iter=10)
"""
