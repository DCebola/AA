...
def generate_Spectral_clusters(_data, kmeans_iterations, _true_labels):
    _metrics = []
    _clusters = []
    for n in range(2, kmeans_iterations + 1):
        _clusters = SpectralClustering(n_init=n * 50, n_clusters=3).fit_predict(_data)
        for m in cluster_eval(_data, _clusters, _true_labels, n):
            _metrics.append(m)
    return _metrics, _clusters
...
def experiment(_name, _feats, _labels, feature_selection=False, corr_filter=False, cluster_iter=0):
    ...
    if feature_selection:
        ...
        #SPECTRAL_SELECTED, spectral_metrics = sequential_backward_elimination(data_df, _labels[LABELED][:, 1],
        #                                                                      SpectralClustering(3, n_init=100, assign_labels="kmeans"),
        #                                                                      heuristic)
       ...
       # plot_cluster_line_metrics(pd.DataFrame(spectral_metrics, columns=METRICS_COLS),
    ...
    if corr_filter:
        ...
        if feature_selection:
            ...
            #spectral_data_df = bisec_data_df.iloc[:, filter_low_high_corr(spectral_data_df.corr())[0]]
        else:
            kmeans_data_df = data_df
            dbscan_data_df = data_df
            bisec_data_df = data_df
    ...
    _spectral_clusters = SpectralClustering(n_clusters=3).fit_predict(spectral_data_df)
    ...
    plot_joint_plot(_spectral_plt_data, "spectral/" + _name + "_clusters", "Spectral")
    ...
    if cluster_iter > 0:
        ...
        plot_cluster_line_metrics(spectral_metrics_df, "/spectral/" + _name + "_spectral_parameter_metrics", "KMeans Iterations",
                                  "Bisecting KMeans Cluster Metrics")
    # ________________________________________________________________________________________________________________________________________________
